...
public class JsonTest {
    ...
    public void typeTest() throws Exception {
        ...
        String out = JsonWriter.writeToString(jctx, grid);
        System.out.println(out);

        // Read the content back in.
        JsonReader reader = new JsonReader(jctx, out);
        HGrid in = reader.read(HGrid.class);
    ...
    public void nestedTest() throws Exception {
        HList list = new HList().add("asdf").add(123);

        HMap inner = new HMap().put("myMarker").put("a", "b").put("list", list);
        HMap outer = new HMap().put("anotherMarker").put("myMap", inner);

        HGrid grid = new HGrid(outer);
